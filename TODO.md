...
## Festa della regione "che esce dall'Italia" (idea del 20/09/2026, da fare)
- [ ] Sagoma dell'Italia con le regioni in coordinate condivise (oggi `src/data/regionShapes.ts` ha una regione alla volta, ognuna con il suo sistema). Rigenerare con `scripts/generate-region-shapes.ts` dai confini ISTAT (serve il GeoJSON originale, non è nel repo: ricercarlo o riscaricarlo), una proiezione unica e le regioni una accanto all'altra
- [ ] Sequenza: mappa spenta con le regioni già d'oro → la regione nuova si illumina e ha la scia di stelline → si stacca (ombra, cresce, vola al centro, lascia il buco) → diventa la figurina d'oro che gira (già fatta in `FoodRain.tsx`) → alla fine la regione d'oro torna al suo posto. Isole e regioni piccole (Valle d'Aosta, Molise) crescono di più
- [ ] Bonus: la stessa mappa in Stats con le regioni che si colorano (grigie, in corso, d'oro)
- Attenzione al peso sul telefono: solo trasformazioni e opacità
