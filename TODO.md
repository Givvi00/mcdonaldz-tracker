...
- [x] **Raccolta automatica delle novità/chiusure** (oggi serve una raccolta a mano): azione programmata su GitHub che interroga OpenStreetMap (Overpass) e unisce con `fetch-data`. Confronto fatto il 19/09/2026: 787 punti OSM vs 828 nel nostro elenco; il 98% dei punti OSM coincide con un nostro ristorante (entro 150 m), ma il **10% dei nostri (81) non ha nessun punto OSM**: l'assenza in OSM NON significa chiuso. Regole prudenti: chiuso solo se era già visto in OSM e manca per più controlli di fila; nuovo solo se compare in modo stabile; soglia di variazione → nessuna scrittura automatica; prime settimane in sola modalità rapporto. In OSM mancano città e via (278/340 su 787): per i nuovi vanno ricavate a parte. Attenzione alla licenza ODbL (attribuzione © OpenStreetMap contributors) — FATTO il 23/09/2026 in sola modalità rapporto: ogni lunedì .github/workflows/osm-check.yml (npm run osm-check, scripts/lib/osmCompare.ts, test in scripts/test-osm.ts) apre o aggiorna una issue con etichetta osm-check se trova possibili nuovi, chiusi o riaperture; non scrive mai l’elenco. Regole: chiuso solo se OSM lo aveva e manca da 2 controlli; nuovo solo se a più di 300 m da tutti i nostri per 2 controlli (OSM disegna lo stesso Mc più volte: edificio + punto, PlayPlace, centri commerciali); primo giro fatto in locale il 23/09 (754 dei nostri trovati su OSM, 11 punti lontani da tutti: se restano, compariranno nella prima segnalazione di lunedì). Stato in shared/data/osm-state.json. Procedura in docs/AGGIORNAMENTO-DATI.md
- [x] Aggiornamenti della web app in silenzio (24/09/2026): la versione nuova si applica da sola quando l’app va in sottofondo (non durante una verifica della posizione), al ritorno compare "App aggiornata" per qualche secondo; niente più banner "Aggiorna". Da Profilo resta "Controlla aggiornamenti". Aprire l’app da chiusa caricava già la versione nuova
...
- [x] La verifica non usa più la posizione letta all'apertura (vecchia e poco precisa): subito dopo aver segnato la visita, in sottofondo, una lettura nuova ad alta precisione; vale solo entro 200 m e con precisione migliore di 150 m (era 100: dentro un edificio il telefono raramente fa meglio; il 22/09 il PC dava "non riesco a capire bene dove sei"); ascolta il telefono fino a 10 s e tiene la lettura migliore, fermandosi appena basta (`src/services/gpsCheck.ts`, test in `scripts/test-gps.ts`). Le visite verificate prima di oggi restano verificate
...
- [x] (22/09/2026) Titoli di sezione uguali ovunque (Home, Stats, Profilo, Installa l’app): icona di cibo disegnata in cerchio giallo, componente SectionTitle.tsx. Da guardare sul telefono
- [x] (23/09/2026) Avvisi sovrapposti: il banner "Nuova versione" (in basso) non si scontra più con l'avviso di verifica (in alto); quello di verifica cede il posto all'avviso di un timbro sbloccato
