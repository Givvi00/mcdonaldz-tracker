...
- [x] Migrazioni del database locale (21/09/2026): `DB_VERSION` e un passo per versione in `src/services/db.ts` (`MIGRATIONS`, `BACKUP_MIGRATIONS`); prima di ogni aggiornamento della struttura i dati vengono copiati in localStorage (`getPreMigrationBackup()`); i backup portano `schemaVersion` e un backup di una versione più nuova viene rifiutato con un messaggio chiaro. Test: `scripts/test-migration.ts` (fake-indexeddb). Ancora da fare: un pulsante nel Profilo per recuperare la copia di sicurezza, se mai servisse
