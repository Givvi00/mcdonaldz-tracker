...
export type RegionTier = 'gold' | 'silver' | 'progress' | 'empty';
...
export function regionSummaries(mcdonalds: McDonald[], visits: Visit[]): RegionSummary[] {
  ...
  for (const mc of countedMcdonalds(mcdonalds, visits)) {
    const entry = byRegion.get(mc.region) ?? { total: 0, visited: 0 };
    entry.total += 1;
...
/** Gold: complete now. Silver: was complete, something new to visit. Otherwise in progress or not started. */
